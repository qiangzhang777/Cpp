...
class trend_one:
    def __init__(self, code, obj, ids_selfBk_lst):
        ...
        # 创建日志句柄
        if not os.path.exists(f"./find_trend/k_daily{self.yestoday}/log/ids/"):
            os.mkdir(f"./find_trend/k_daily{self.yestoday}/log/ids/")

        # 个股句柄
        self.ids_log = open(f"./find_trend/k_daily{self.yestoday}/log/ids/{self.code}.txt", "a+")
        ...
        try:
            ...
            if buy_cond1:
                ...
                if (curr_dp_info['sh']['curr_macd'] < 0 and curr_dp_info['sh']['curr_dif'] < 0) or (curr_dp_info['sh']['mk_kline_macd'] == 'desc') or (self.dapan_yes1_beili == 'top'):
                    ...
                    self.money1 = self.per_top_money * 5
                    self.money2 = self.per_top_money * 10
                    self.money3 = 0
                    self.money4 = 0
                    self.money5 = 0
                ...
                if sale_cond1 or (curr_time.hour == 14 and curr_time.minute >= 43):
                    ...
                    try:
                        ...
                        if len(self.sold_min_dct['curr_price']) >= 28:
                            price_trend = self.mk(self.sold_min_dct['curr_price'])[0]
                    ...
                    elif (r['highest_price'] - curr_price) / r['highest_price'] > 0.00368 and ((price_trend == "decs" and ids_mMacd_trd == 'decs') or (price_trend == "decs" and np.isnan(ids_mMacd) and (curr_time.hour == 9 and curr_time.minute <= 40))):
                        if r['st'] == 0:
                            try:
...
class Trend:
    def __init__(self, today, yestoday, run_type, jk_buy_lst=None):
                ...
                if self.run_type in ['trading', 'trading_debug']:
                    try:
                        if os.path.exists(f'{self.data_path}/bk_kline/{per.code}.xlsx'):
                            self.bk_kline[per.code] = pd.read_excel(f'{self.data_path}/bk_kline/{per.code}.xlsx')
                            self.bk_close_lst[per.code] = copy.deepcopy(self.bk_kline[per.code].iloc[::-1]['close'].values)
                    except Exception as e:
                        self.logger.error(e)
                elif self.run_type == 'backtest':
                    pass
                    ...
                    # print(self.bk_kline[per.code].tail(5)[['name', 'trade_date', 'close', 'yes1_macd']])
        ...
        if self.run_type == 'backtest':
            ...
            for per_bk in self.bk_jk_df.itertuples():
                ...
                # ratio = /len(self.all_jk_buy_list)
                idsNums_allowed_buy = int(bk_ids_nums * 0.3)
                # print()
        ...
        # 创建日志句柄
        if not os.path.exists(f"./find_trend/k_daily{self.yestoday}/log/"):
            os.mkdir(f"./find_trend/k_daily{self.yestoday}/log/")

        if not os.path.exists(f"./find_trend/k_daily{self.yestoday}/log/ids/"):
            os.mkdir(f"./find_trend/k_daily{self.yestoday}/log/ids/")

        # 大盘句柄
        self.dp_log = open(f"./find_trend/k_daily{self.yestoday}/log/log_dp.txt", "a+")
        # 板块句柄
        self.bk_log = open(f"./find_trend/k_daily{self.yestoday}/log/log_bk.txt", "a+")
    ...
    def get_basic_jk_sale(self):
        ...
        while True:
            ...
            # 每秒获取逐笔交易数据
            curr_time = datetime.datetime.now()
            time_cond = (curr_time.hour >= 9 and curr_time.hour < 15) and ((curr_time.hour == 9 and curr_time.minute >= 29) or curr_time.hour >= 10)
            # time_cond = True
            ...
            if self.run_type == 'trading_debug' or (time_cond and self.run_type == 'trading'):
                ...
                try:
                    ...
                    if self.run_type == 'trading_debug':
                        ...
                        for c in self.all_jk_sale_list:
                            # print(c)
                            ncode.append(self.chg_code_type(c))
                        all_list_2str = ','.join(ncode)

                        if len(all_list_2str) == 0:
                            continue
                        # print('all_list_2str', all_list_2str)
                    ...
                    for code in self.all_jk_sale_list:
                        try:
                            if code in buy_obj.keys():
                                ...
                                print(code, 'jk_sale:实力化失败，没有进入异步卖出函数')
                        except Exception as e:
                            # raise e
                            self.logger.error(e)
            ...
            elif curr_time.hour <= 8 or curr_time.hour >= 15 and curr_time.minute >= 20:
                print('Info: not in trading time, break jk sale loop', curr_time)
                break
    ...
    # 处理买入
    def get_basic_jk_buy(self, path):

        # 启动时，开始find_trend
        # if self.run_type in ['trading', 'trading_debug']:
        #     is_panzhong = True
        #     ft = find_trend5.find_trend(type, path, self.yestoday, self.today, is_panzhong)
        #     ft.getBaseCode()

        # 获取到find_trend之后，读取ids数据到 trend 对象中, 并预选出符合条件的ids进行监控
        idsDf = pd.read_excel(f'./find_trend/k_daily{self.yestoday}/000trend_k_list.xlsx')
        jk_idsDf = copy.deepcopy(idsDf[(idsDf['close'] < 120) & ((idsDf['macd_trend3'] == 'incs') & (idsDf['macd_trend4'] != 'decs')) &
                                  (idsDf['beili'] != 'top') &
                                  (idsDf['yes1_macd'] > idsDf['yes2_macd']) & (idsDf['yes1_macd'] > 0) & (idsDf['dif1'] > 0) & (idsDf['dea1'] > 0) &
                                  (idsDf['pct_chg'] < 6) & (idsDf['pct_chg'] > -4) &
                                  ((idsDf['yes1_highest'] - idsDf['yes1_close']) / idsDf['yes1_highest'] < 0.036)])

        # jk_idsDf = pd.read_excel(f'./find_trend/k_daily{self.yestoday}/jk_ids_all.xlsx')

        self.logger.info(f'jk_idsDf len is: {len(jk_idsDf)}')

        # 把jk_idsDf,按板块展开

        if os.path.exists(f'./find_trend/k_daily{self.yestoday}/bk_ids_1v1_{self.today}.xlsx'):
            self.logger.info(f'bk_ids_1v1_{self.today}.xlsx is exists read to bk_ids_1v1_df')
            bk_ids_1v1_df = pd.read_excel(f'./find_trend/k_daily{self.yestoday}/bk_ids_1v1_{self.today}.xlsx')
        else:
            bk_ids_1v1_df = pd.DataFrame()
            for ids in jk_idsDf.itertuples():
                bk_lst = self.bk_code[self.bk_code['code'] == ids.ts_code]['bk_code'].values
                for bk_code in bk_lst:
                    jk_idsDf.loc[ids.Index, 'bk_code'] = bk_code
                    bk_ids_1v1_df = bk_ids_1v1_df.append(jk_idsDf.loc[ids.Index], ignore_index=True)

                print('expand ids_bk:', bk_lst)
            bk_ids_1v1_df.to_excel(f'./find_trend/k_daily{self.yestoday}/bk_ids_1v1_{self.today}.xlsx')

        # 实列化code对象
        buy_obj = {}
        for code in jk_idsDf['ts_code'].values:
            try:
                ids_selfBk_lst = bk_ids_1v1_df[bk_ids_1v1_df['ts_code'] == code]['bk_code'].values
                ids_selfBk_name_lst = self.bk_ids_1v1_df[self.bk_ids_1v1_df['ts_code'] == code]['bk_name'].values
                buy_obj[code] = trend_one(code, self, ids_selfBk_lst)
                self.logger.info(f'实列化{code},成功')
            except Exception as e:
                # print(code, self.code_ma)
                # raise e
                self.logger.error(f'实列化{code},失败:{e}')
            ...
            if self.run_type == 'trading_debug' or (time_cond and self.run_type == 'trading'):
                ...
                try:
                    with self.dp_bk_lock:
                        curr_bk_info = copy.deepcopy(self.curr_bk_info)
                        curr_dp_info = copy.deepcopy(self.curr_dp_info)
                    dp_curr_cond_bad = (curr_dp_info['sh']['curr_macd'] < 0 and curr_dp_info['sh']['curr_dif'] < 0) or (curr_dp_info['sh']['mk_kline_macd'] == 'decs') or (self.dapan_yes1_beili == 'top')
                    dp_curr_cond_good = (curr_dp_info['sh']['curr_macd'] > 0 and curr_dp_info['sh']['curr_dif'] > 0) and (curr_dp_info['sh']['mk_kline_macd'] == 'incs') and (self.dapan_yes1_beili != 'top')
                except Exception as e:
                    self.logger.error(f'continue in error:{e}')
                else:
                    # 在尾盘遍历板块情况，然后筛选出板块中符合条件的个股，实例化，然后进行买入
                    all_bk_ids_lst = []
                    all_bk_dct = {}
                    for bk_code, per_bk in curr_bk_info.items():
                        # 3分钟更新一次，否则太频繁了
                        if curr_time.minute % 3 == 0 and curr_time.second % 59 == 0:
                            br = self.bk_ids.find_one({'date': self.today, 'bk_code': bk_code})
                            bk_curr_cond = curr_bk_info[br['bk_code']]['mk_green_red_value'] < -4.18 and curr_bk_info[br['bk_code']]['green_red'] < 0.28 and curr_bk_info[br['bk_code']]['mk_chg_pct'] > 4.18
                            if dp_curr_cond_bad and (not (bk_curr_cond)):
                                ...
                                if idsNums_allowed_buy != br['idsNums_allowed_buy']:
                                    self.logger.info(f"{br['bk_code']}chg idsNums_allowed_buy {br['idsNums_allowed_buy']} to {idsNums_allowed_buy}")
                        ...
                        if per_bk['trd_state'] == True:
                            # 当前板块可以购买的个股
                            all_bk_ids_lst = all_bk_ids_lst + list(bk_ids_1v1_df[bk_ids_1v1_df['bk_code'] == bk_code]['ts_code'].values)
                            all_bk_dct[per_bk['bk_code']] = per_bk['bk_name']
                    if len(all_bk_ids_lst) == 0:
                        continue

                    print('all_bk_dct:', all_bk_dct)
                    print("all_bk_ids_lst:", all_bk_ids_lst)
                    for j in range(0, len(all_bk_ids_lst), 30):

                        bk_ids_lst = all_bk_ids_lst[j:j+30]
                        ...
                        if len(bk_ids_lst) == 0:
                            ...
                            try:
                                ...
                                tasks_buy = []
                                # print('creat tasks', bk_ids_lst)
                                for code in bk_ids_lst:
                                    try:
                                        if (not curr_bk_info) or (not curr_dp_info):
                                            continue
                                        if (code not in self.lastreq.keys()) or (code not in buy_obj.keys()):
                                            continue
    ...
    def get_basic_backtest_data(self):
        ...
        for code in self.all_jk_list:
            ...
            if os.path.exists(file_path):
                try:
                    ...
                    # 计算分时macd数据
                    dif, dea, dw = talib.MACD(temp_df['curr_price'], 720, 1560, 540)
                    macd = (dif - dea) * 2
                    temp_df['dif'] = dif
                    temp_df['dea'] = dea
                    temp_df['macd'] = macd
                    ...
                    back_data_df['curr_price'] = temp_df['curr_price']
                    # back_data_df = pd.merge(back_data_df, temp_df, how='inner')
                    back_data_df = back_data_df.append(temp_df, ignore_index=True)
                    print(back_data_df)
        ...
        for t in range(93000, 150001):
            ...
            try:
                ...
                print(per)
                continue
                if int(t) > 113000 and int(t) < 130000:
                    continue
                curr_time = self.today + str(int(t))
                del per['time']
                self.ser = per
...
def main(run_type):
    if run_type == 'backtest':
        ...
        today = "2023-03-03"
        yestoday = '2023-03-02'
        path = './find_trend/k_daily' + today

        if not os.path.exists(path):
            os.mkdir(path)
        if not os.path.exists(path+'/bk_kline/'):
            os.mkdir(path+'/bk_kline/')
        if not os.path.exists(path+'/code_kline/'):
            os.mkdir(path+'/code_kline/')
        ...
        if run_type == 'trading_debug':
            trend.get_basic_jk_buy(path)
            trend.get_basic_jk_sale()
            ...
            thr_dct[1] = thr.Thread(target=trend.get_basic_jk_buy, args=(path, ))
            thr_dct[2] = thr.Thread(target=trend.get_basic_jk_sale)
            thr_dct[3] = thr.Thread(target=trend.get_dp_bk_data)
            thr_dct[4] = thr.Thread(target=trend.keep_login)
            thr_dct[5] = thr.Thread(target=trend.get_reality_cangwei)
